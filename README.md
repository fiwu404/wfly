...
Windows x64 轻量代理桌面客户端，当前版本：`v0.0.3`。
...
- 首页提供节点状态、图形化三档模式开关、用户触发的出口 IP / Google 延迟检测，以及四条可悬停读取的流量曲线。
...
5. 在首页选择节点和模式，然后启动。系统代理只会在你选中“系统代理”且使用 WFly 生成的 sing-box 配置时指向本机 `127.0.0.1`；关闭/退出时仅在设置仍由 WFly 持有时恢复原有值。
